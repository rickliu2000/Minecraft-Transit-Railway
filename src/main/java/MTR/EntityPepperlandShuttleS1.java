...
public class EntityPepperlandShuttleS1 extends EntityTrainBase {

	private static final DataParameter<Integer> PS_ROUTE = EntityDataManager.<Integer>createKey(EntityTrainBase.class,
			DataSerializers.VARINT);

	public EntityPepperlandShuttleS1(World world) {
		super(world);
		doorOpen = MTRSounds.lightrail1Dooropen;
		doorClose = MTRSounds.lightrail1Doorclose;
	}

	public EntityPepperlandShuttleS1(World world, double x, double y, double z, boolean f, int h) {
	...
	private void setRouteBehind() {
		try {
			EntityPepperlandShuttleS1 entityBack;
			MinecraftServer server = FMLCommonHandler.instance().getMinecraftServerInstance().getServer();
			if (front)
				entityBack = (EntityPepperlandShuttleS1) server.getEntityFromUuid(uuidWheel);
			else
				entityBack = (EntityPepperlandShuttleS1) server.getEntityFromUuid(uuidConnected);
