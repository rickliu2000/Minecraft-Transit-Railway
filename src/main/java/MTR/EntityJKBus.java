...
public class EntityJKBus extends EntityTrainBase {

	private static final DataParameter<Integer> JKBUS_ROUTE = EntityDataManager.<Integer>createKey(EntityTrainBase.class,
			DataSerializers.VARINT);

	public EntityJKBus(World world) {
		super(world);
		doorOpen = MTRSounds.lightrail1Dooropen;
		doorClose = MTRSounds.lightrail1Doorclose;
	}

	public EntityJKBus(World world, double x, double y, double z, boolean f, int h) {
	...
	private void setRouteBehind() {
		try {
			EntityJKBus entityBack;
			MinecraftServer server = FMLCommonHandler.instance().getMinecraftServerInstance().getServer();
			if (front)
				entityBack = (EntityJKBus) server.getEntityFromUuid(uuidWheel);
			else
				entityBack = (EntityJKBus) server.getEntityFromUuid(uuidConnected);
	...
	private void setRoute(int route) {
		dataManager.set(JKBUS_ROUTE, route);
	}

	public int getRoute() {
		return dataManager.get(JKBUS_ROUTE);
	}
	...
	protected void entityInit() {
		super.entityInit();
		dataManager.register(JKBUS_ROUTE, new Integer(0)); // route number
	}
