...
public class MTRDispenserBehavior {
    ...
    public static class DispenseJKBus extends BehaviorDefaultDispenseItem {
        @Override
        protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
            World worldIn = source.getWorld();
            EnumFacing facing = source.getBlockState().getValue(BlockDispenser.FACING);
            ...
            if (!(worldIn.getBlockState(pos).getBlock() instanceof BlockRailBooster))
                pos = source.getBlockPos().offset(facing, 2);
            if (!worldIn.isRemote && worldIn.getBlockState(pos).getBlock() instanceof BlockRailBooster)
                MTRItems.itemjkbus.onItemUse(stack, null, worldIn, pos, EnumHand.MAIN_HAND, facing, pos.getX(),
                                                  pos.getY(), pos.getZ());
            return stack;
        }
    }
    ...
    public static class DispensePepperlandShuttleS1 extends BehaviorDefaultDispenseItem {
        @Override
        protected ItemStack dispenseStack(IBlockSource source, ItemStack stack) {
            World worldIn = source.getWorld();
            EnumFacing facing = source.getBlockState().getValue(BlockDispenser.FACING);
            ...
            if (!(worldIn.getBlockState(pos).getBlock() instanceof BlockRailBooster))
                pos = source.getBlockPos().offset(facing, 2);
            if (!worldIn.isRemote && worldIn.getBlockState(pos).getBlock() instanceof BlockRailBooster)
                MTRItems.itemPepperlandShuttleS1.onItemUse(stack, null, worldIn, pos, EnumHand.MAIN_HAND, facing, pos.getX(),
                                                         pos.getY(), pos.getZ());
            return stack;
        }
    }
