...
public class RenderPepperlandShuttleS1 extends RenderTrain<EntityPepperlandShuttleS1> {

	private ModelPepperlandShuttleS1 model = new ModelPepperlandShuttleS1();
    //private ModelLightRail1 model = new ModelLightRail1();

	public RenderPepperlandShuttleS1(RenderManager renderManager) {
		super(renderManager);
	}

	@Override
	protected ResourceLocation getEntityTexture(EntityPepperlandShuttleS1 entity) {
        ...
        ResourceLocation skin = new ResourceLocation("mtr:textures/entity/skins/s1/JiangNanS1.png");
        if (isCB){
            skin = new ResourceLocation("mtr:textures/entity/skins/s1/JiangNanS1.png");
        } else if (isJiangNan){
            skin = new ResourceLocation("mtr:textures/entity/skins/s1/JiangNanS1.png");
        }
        return skin;
	...
	@Override
	protected void render(EntityPepperlandShuttleS1 entity, float leftDoor, float rightDoor) {
		int route = entity.getRoute();
		boolean p;
		boolean down = route < 0;
		route = Math.abs(route);
	...
	private ResourceLocation getSignTexture(int route, boolean p, int n) {
		ResourceLocation texture = new ResourceLocation(
				"mtr:textures/displays/bus/s1/bus-" + route + (p ? "P-" : "-") + n + ".png");
		try {
			Minecraft.getMinecraft().getResourceManager().getResource(texture);
		} catch (IOException e) {
			texture = new ResourceLocation("mtr:textures/entity/s1/bus-default-" + n + ".png");
		}
		return texture;
	}
}
